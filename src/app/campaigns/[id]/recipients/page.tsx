  ...
  return (
    <>
      ...
      {rows.length === 0 ? (
        <div className="mt-4">
          <EmptyState
            title={campaign.status === "draft" ? "No recipients prepared yet" : "No campaign recipients found"}
            detail={
          ...
          <div className="mt-3 flex gap-3 text-sm">
            <Link href={`/campaigns/${id}`} className="text-accent hover:underline">Open overview</Link>
            <Link href={`/campaigns/${id}/variants`} className="text-accent hover:underline">Check variants</Link>
            <Link href={`/campaigns/${id}/waves`} className="text-accent hover:underline">Check waves</Link>
          </div>
        </div>
      ) : (
