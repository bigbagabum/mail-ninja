  ...
  return (
    <>
      ...
      <form action={createCampaignAction} className="grid max-w-2xl gap-4 rounded border border-line bg-white p-5">
        ...
        <label className="text-sm font-medium">From name<input name="fromName" defaultValue={env.DEFAULT_FROM_NAME} required className="mt-1 w-full rounded border-line" /></label>
        <label className="text-sm font-medium">From email<input name="fromEmail" type="email" defaultValue={env.DEFAULT_FROM_EMAIL} required className="mt-1 w-full rounded border-line" /></label>
        <label className="text-sm font-medium">Reply-to<input name="replyTo" type="email" defaultValue={env.DEFAULT_REPLY_TO} className="mt-1 w-full rounded border-line" /></label>
        <button className="w-fit rounded bg-accent px-3 py-2 text-sm font-medium text-white">Create campaign</button>
      </form>
    </>
  );
}
