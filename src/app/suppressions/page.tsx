...
export default async function SuppressionsPage() {
  ...
  return (
    <>
      <PageHeader title="Audience Exclusions" />
      <AudienceNav />
      <p className="mb-4 text-sm text-muted">
        Exclusions are email addresses that must not receive campaigns, such as
        unsubscribes, complaints, hard bounces or manual blocks.
      </p>
      <form
        action={addSuppressionAction}
        className="mb-6 grid gap-3 rounded border border-line bg-white p-4 md:grid-cols-[1fr_180px_120px_auto]"
      >
        <input
          name="email"
          type="email"
        ...
        />
        <select name="reason" className="rounded border-line">
          ...
          <tbody>
            {rows.map((row) => (
              <tr key={row.id} className="border-t border-line">
                ...
                <td>
                  {row.isPermanent ? (
                    <Badge tone="warn">permanent</Badge>
                  ) : (
                ...
                <td className="p-3">
                  ...
                  >
                    <input type="hidden" name="suppressionId" value={row.id} />
                    {row.isPermanent ? (
                      <label className="flex items-center gap-1 text-xs text-muted">
                        <input
                          type="checkbox"
                          name="confirmPermanentRemoval"
                          value="true"
                          className="rounded border-line"
                        />
                        Confirm
