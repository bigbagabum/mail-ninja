...
export async function resetAdminPasswordAction(formData: FormData) {
  ...
  const target = await db.query.adminUsers.findFirst({
    where: and(
      eq(adminUsers.id, data.adminUserId),
      eq(adminUsers.workspaceId, current.workspaceId),
    ),
  });
  ...
  await db.transaction(async (tx) => {
    ...
    await tx
      ...
      .where(eq(adminSessions.adminUserId, target.id));
...
export async function changeOwnPasswordAction(formData: FormData) {
  ...
  await db.transaction(async (tx) => {
    ...
    await tx
      .delete(adminSessions)
      .where(eq(adminSessions.adminUserId, current.id));
    await tx.insert(auditLogs).values({
      workspaceId: current.workspaceId,
      adminUserId: current.id,
...
export async function setAdminStatusAction(formData: FormData) {
  ...
  await db.transaction(async (tx) => {
    await tx
      .update(adminUsers)
      .set({ isActive: data.isActive === "true", updatedAt: new Date() })
      .where(eq(adminUsers.id, target.id));
