...
import { extractClickedUrl, processEmailEvent } from "@/server/webhooks/events";
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (secrets.length === 0)
      throw new Error("No Resend webhook secret is configured.");
    let verified: Awaited<
      ReturnType<typeof verifyResendWebhookWithSecret>
    > | null = null;
    let providerAccountId: string | null = null;
    for (const candidate of secrets) {
      try {
        verified = await verifyResendWebhookWithSecret({
          payload,
          headers,
    ...
    const existing = await db.query.emailEvents.findFirst({
      where: eq(emailEvents.providerEventId, verified.providerEventId),
    });
    ...
    logger.warn("resend webhook rejected", {
