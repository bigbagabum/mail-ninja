...
export class ResendEmailCampaignProvider implements EmailCampaignProvider {
  ...
  constructor(input?: { apiKey?: string; providerAccountId?: string | null }) {
    const apiKey =
      input?.apiKey ?? env.RESEND_API_KEY ?? "re_webhook_verification_only";
  ...
  async verifyWebhook(
    ...
    return {
      providerEventId:
        input.headers.id ?? String(payload.id ?? crypto.randomUUID()),
  ...
  async createSegment(input: CreateSegmentInput): Promise<ProviderSegment> {
    const { data, error } = await this.client.segments.create({
      name: input.name,
    });
    if (error) throw new Error(error.message);
    await markProviderAccountUsed(this.providerAccountId);
  ...
  async addContactsToSegment(
    input: AddContactsToSegmentInput,
  ): Promise<AddContactsResult> {
    return {
      added: input.contactIds.length,
      raw: {
        note: "Segment membership reconciliation is provider-version dependent.",
  ...
  async sendBroadcast(input: SendBroadcastInput): Promise<SendBroadcastResult> {
    const { data, error } = await this.client.broadcasts.send(
      input.broadcastId,
    );
    if (error) throw new Error(error.message);
    return { id: idFrom(data), raw: data };
...
export async function verifyResendWebhookWithSecret(
  input: VerifyWebhookInput & { webhookSecret: string },
) {
...
export async function createProviderForWorkspace(workspaceId: string): Promise<{
  ...
  return {
    provider: new ResendEmailCampaignProvider({
