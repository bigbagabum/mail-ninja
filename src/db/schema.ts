...
export const providerRoutingStrategyEnum = pgEnum("provider_routing_strategy", ["sequential", "parallel"]);
export const providerMetricsModeEnum = pgEnum("provider_metrics_mode", ["combined", "by_provider_account"]);
export const providerAccountStatusEnum = pgEnum("provider_account_status", ["active", "paused", "failed"]);
...
export const adminSessions = pgTable(
  ...
  (table) => ({ expiresIdx: index("admin_sessions_expires_idx").on(table.expiresAt) })
);
...
export const workspaceSettings = pgTable("workspace_settings", {
  ...
  providerRoutingStrategy: providerRoutingStrategyEnum("provider_routing_strategy").notNull().default("sequential"),
  providerMetricsMode: providerMetricsModeEnum("provider_metrics_mode").notNull().default("combined"),
...
export const providerAccounts = pgTable(
  ...
  (table) => ({
    workspaceProviderIdx: index("provider_accounts_workspace_provider_idx").on(table.workspaceId, table.provider, table.status),
...
export const recipients = pgTable(
  ...
  (table) => ({
    ...
    priorityIdx: index("recipients_priority_idx").on(table.workspaceId, table.priorityScore, table.id)
  })
);
...
export const imports = pgTable("imports", {
  ...
  importOptions: jsonb("import_options").$type<Record<string, unknown>>().default({}).notNull(),
...
export const importRows = pgTable(
  "import_rows",
  {
    ...
    duplicateOfRecipientId: uuid("duplicate_of_recipient_id").references(() => recipients.id),
    recipientId: uuid("recipient_id").references(() => recipients.id),
    createdAt: timestamp("created_at", { withTimezone: true }).defaultNow().notNull()
  },
  (table) => ({ uniqRow: unique().on(table.importId, table.rowNumber), importIdx: index("import_rows_import_idx").on(table.importId) })
);
...
export const campaigns = pgTable(
  ...
  (table) => ({ uniq: unique().on(table.workspaceId, table.campaignKey), workspaceIdx: index("campaigns_workspace_idx").on(table.workspaceId) })
);
...
export const campaignVariants = pgTable(
  ...
  (table) => ({ uniq: unique().on(table.campaignId, table.locale, table.recipientRole) })
);
...
export const campaignRecipients = pgTable(
  "campaign_recipients",
  {
    id: uuid("id").primaryKey().defaultRandom(),
    campaignId: uuid("campaign_id").references(() => campaigns.id, { onDelete: "cascade" }).notNull(),
    recipientId: uuid("recipient_id").references(() => recipients.id, { onDelete: "cascade" }).notNull(),
    variantId: uuid("variant_id").references(() => campaignVariants.id, { onDelete: "restrict" }),
    waveId: uuid("wave_id").references(() => campaignWaves.id, { onDelete: "set null" }),
  ...
  (table) => ({
    ...
    messageIdx: index("campaign_recipients_message_idx").on(table.providerMessageId)
  })
);
...
export const providerSegments = pgTable("provider_segments", {
  ...
  workspaceId: uuid("workspace_id").references(() => workspaces.id, { onDelete: "cascade" }).notNull(),
  campaignId: uuid("campaign_id").references(() => campaigns.id, { onDelete: "cascade" }).notNull(),
  waveId: uuid("wave_id").references(() => campaignWaves.id, { onDelete: "cascade" }).notNull(),
  variantId: uuid("variant_id").references(() => campaignVariants.id, { onDelete: "cascade" }).notNull(),
...
export const providerBroadcasts = pgTable(
  "provider_broadcasts",
  {
    ...
    variantId: uuid("variant_id").references(() => campaignVariants.id, { onDelete: "cascade" }).notNull(),
    providerAccountId: uuid("provider_account_id").references(() => providerAccounts.id, { onDelete: "set null" }),
    providerSegmentId: uuid("provider_segment_id").references(() => providerSegments.id, { onDelete: "restrict" }),
    provider: text("provider").notNull(),
  ...
  (table) => ({ uniq: unique().on(table.campaignId, table.waveId, table.variantId) })
);
...
export const emailEvents = pgTable(
  ...
  (table) => ({
    ...
    providerAccountIdx: index("email_events_provider_account_idx").on(table.providerAccountId)
  })
);
...
export const suppressions = pgTable(
  ...
  (table) => ({
    uniq: unique().on(table.workspaceId, table.normalizedEmail),
    activeIdx: index("suppressions_active_idx").on(table.workspaceId, table.normalizedEmail, table.expiresAt)
  })
);
...
export const jobs = pgTable(
  ...
  (table) => ({
    claimIdx: index("jobs_claim_idx").on(table.status, table.runAfter, table.priority)
  })
);
...
export const campaignAnalytics = pgTable(
  "campaign_analytics",
  {
    id: uuid("id").primaryKey().defaultRandom(),
    campaignId: uuid("campaign_id").references(() => campaigns.id, { onDelete: "cascade" }).notNull(),
    waveId: uuid("wave_id").references(() => campaignWaves.id, { onDelete: "cascade" }),
    variantId: uuid("variant_id").references(() => campaignVariants.id, { onDelete: "cascade" }),
  ...
  (table) => ({ uniq: unique().on(table.campaignId, table.waveId, table.variantId, table.dimensionType, table.dimensionValue) })
);
