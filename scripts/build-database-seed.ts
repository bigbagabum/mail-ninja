...
const outputPath = path.join(rootDir, "docs", "database-seed.sql");
...
function buildSeedSql() {
  return `-- Mail Ninja database seed
...
if (checkOnly) {
  ...
  if (current !== seedSql) {
    console.error(
      `Database seed file is out of date. Run npm run db:seed-sql.`,
    );
    process.exit(1);
  }
  console.log(`Database seed file is up to date: ${outputPath}`);
} else {
  await writeFile(outputPath, seedSql);
  console.log(`Database seed written: ${outputPath}`);
}
